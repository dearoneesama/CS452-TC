...
static constexpr char ENABLE_CTS_MASK = 0b1000;
static constexpr char ENABLE_TX_AND_CTS_MASK = ENABLE_TX_MASK | ENABLE_CTS_MASK;
};
...
void uart_interrupt_state::enable_modem_interrupt() {
  ier1 |= ENABLE_CTS_MASK;
  uart_write_register(0, 1, rpi::UART_IER, ier1);
}

void uart_interrupt_state::disable_modem_interrupt() {
  ier1 &= ~ENABLE_CTS_MASK;
  uart_write_register(0, 1, rpi::UART_IER, ier1);
}
